...
then
    ...
    then
        ...
        echo "deploy_backend_webhook_function.sh failed"
        exit $?
    fi
    ...
    az functionapp config appsettings set --name $BACKEND_WEBHOOK_FUNCTION_APP_NAME --resource-group $INFRASTRUCTURE_RESOURCE_GROUP_NAME --settings AzureWebJobsStorage=${storageConnectionString}
    if [ $? -ne 0 ]
    then
        echo "Could not set $BACKEND_WEBHOOK_FUNCTION_APP_NAME application settings."
        echo "deploy_backend_webhook_function.sh failed"
        exit $?
    fi
    ...
    az functionapp config appsettings set --name $BACKEND_WEBHOOK_FUNCTION_APP_NAME --resource-group $INFRASTRUCTURE_RESOURCE_GROUP_NAME --settings "EVENT_GRID_TOPIC_URI=${topic_uri}"
    if [ $? -ne 0 ]
    then
        echo "Could not set $BACKEND_WEBHOOK_FUNCTION_APP_NAME event grid topic URI."
        echo "deploy_backend_webhook_function.sh failed"
        exit $?
    fi
    ...
    az functionapp config appsettings set --name $BACKEND_WEBHOOK_FUNCTION_APP_NAME --resource-group $INFRASTRUCTURE_RESOURCE_GROUP_NAME --settings "EVENT_GRID_KEY=${topic_key}"
    if [ $? -ne 0 ]
    then
    ...
    az functionapp config appsettings set --name $BACKEND_WEBHOOK_FUNCTION_APP_NAME --resource-group $INFRASTRUCTURE_RESOURCE_GROUP_NAME --settings "REDIS_CONNECTION_STRING=${redis_uri_detail[0]}:${redis_uri_detail[1]},password=$redis_key,ssl=True,abortConnect=False"
    if [ $? -ne 0 ]
    then
        echo "Could assign the $BACKEND_WEBHOOK_FUNCTION_APP_NAME Redis connection string."
        echo "deploy_backend_webhook_function.sh failed"
        exit $?
    fi
fi
